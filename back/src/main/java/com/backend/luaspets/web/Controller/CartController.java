...
public class CartController {
    ...
    // Endpoint para obtener un carrito por userId
    ...
    // Endpoint para agregar un producto al carrito usando userId
    @PostMapping("/user/{userId}/items")
    public ResponseEntity<?> addProductToCartByUserId(
            @PathVariable Integer userId,
            @RequestBody CartItemRequest request) {
        try {
            // Verifica si los valores del cuerpo no son nulos o inválidos
            if (request.getProductId() == null || request.getProductType() == null || request.getQuantity() == null) {
                return ResponseEntity.badRequest().body("Missing required fields: productId, productType, or quantity");
            }
            CartItem addedItem = cartService.addProductToCartByUserId(
                    userId,
                    request.getProductType(),
                    request.getProductId(),
                    request.getQuantity());
    ...
    // Endpoint para obtener todos los productos de un carrito por userId
    @GetMapping("/user/{userId}/items")
    public ResponseEntity<List<CartItem>> getAllItemsInCartByUserId(@PathVariable Integer userId) {
        try {
            List<CartItem> items = cartService.getAllItemsInCartByUserId(userId);
            return ResponseEntity.ok(items);
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<?> removeProductFromCart(@PathVariable Integer cartItemId) {
        try {
            cartService.removeProductFromCart(cartItemId);
            return ResponseEntity.noContent().build();
        } catch (RuntimeException e) {
            return ResponseEntity.status(404).body("CartItem not found: " + e.getMessage());
        }
    }
