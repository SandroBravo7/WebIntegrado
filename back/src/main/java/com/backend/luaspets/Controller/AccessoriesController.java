...
import com.backend.luaspets.Services.AccessoriesService;
...
public class AccessoriesController {
    ...
    @GetMapping
    public ResponseEntity<List<Accessory>> getAllFood() {
        List<Accessory> accessories = accessoriesService.getAllAccessories();
        return ResponseEntity.ok(accessories);
    }

    // Endpoint para obtener un producto por su ID
    @GetMapping("/{id}")
    public ResponseEntity<Accessory> getAccessoriesById(@PathVariable Integer id) {
        Optional<Accessory> accessories = accessoriesService.getAccessoriesById(id);
    ...
    @PostMapping
    public ResponseEntity<Accessory> createAccessories(@RequestBody Accessory accessory) {
        try {
            Accessory newAccessory = accessoriesService.saveAccessories(accessory);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Accessory> updateAccessories(@PathVariable Integer id,
                                                       @RequestBody Accessory accessoryDetails) {
        try {
            Accessory updatedAccessory = accessoriesService.updateAccessories(id, accessoryDetails);
...
public ResponseEntity<byte[]> exportMedicineData() {
    try (Workbook workbook = new XSSFWorkbook()) {
        ...
        List<Accessory> medicines = accessoriesService.getAllAccessories();
        int rowIndex = 2; // Comienza después de la fila del título y el encabezado

        // Llenar la hoja con los datos de los accesorios
        for (Accessory accessory : medicines) {
            Row row = sheet.createRow(rowIndex++);

            // ID
            Cell cellId = row.createCell(0);
            cellId.setCellValue(accessory.getId());
            cellId.setCellStyle(dataStyle);

            // Nombre
            Cell cellName = row.createCell(1);
            cellName.setCellValue(accessory.getName());
            cellName.setCellStyle(dataStyle);

            // Marca
            Cell cellBrand = row.createCell(2);
            cellBrand.setCellValue(accessory.getBrand() != null ? accessory.getBrand() : "");
            cellBrand.setCellStyle(dataStyle);

            // Descripción
            Cell cellDescription = row.createCell(3);
            cellDescription.setCellValue(accessory.getDescription() != null ? accessory.getDescription() : "");
            cellDescription.setCellStyle(dataStyle);

            // Precio
            Cell cellPrice = row.createCell(4);
            cellPrice.setCellValue(accessory.getPrice() != null ? accessory.getPrice().doubleValue() : 0.0);
            cellPrice.setCellStyle(currencyStyle);

            // Stock
            Cell cellStock = row.createCell(5);
            cellStock.setCellValue(accessory.getStock() != null ? accessory.getStock() : 0);
            cellStock.setCellStyle(dataStyle);

            // Categoría
            Cell cellCategory = row.createCell(6);
            cellCategory.setCellValue(accessory.getCategory() != null ? accessory.getCategory() : "");
            cellCategory.setCellStyle(dataStyle);

            // URL de la imagen
            Cell cellImageUrl = row.createCell(7);
            cellImageUrl.setCellValue(accessory.getImage_url() != null ? accessory.getImage_url() : "");
            cellImageUrl.setCellStyle(dataStyle);

            // Fecha de registro
            Cell cellCreatedAt = row.createCell(8);
            cellCreatedAt.setCellValue(accessory.getCreated_at() != null ? accessory.getCreated_at().toString() : "");
            cellCreatedAt.setCellStyle(dataStyle);

            // Fecha de expiración
            Cell cellExpirationDate = row.createCell(9);
            cellExpirationDate.setCellValue(accessory.getExpiration_date() != null ? accessory.getExpiration_date().toString() : "");
